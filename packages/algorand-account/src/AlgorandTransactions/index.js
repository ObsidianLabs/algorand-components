...
export default class AlgorandTransactions extends PureComponent {
  ...
  refresh = async account => {
    ...
    const { cursor, data: txs } = await algorandNode.algoSdk.getTransactions(account.address)
    this.setState({ txs, cursor, hasMore: !!cursor, loading: false })
  }

  loadMore = async () => {
    this.setState({ loading: true })
    const { cursor, data: txs } = await algorandNode.algoSdk.getTransactions(this.props.account.address, this.state.cursor)
    this.setState({
