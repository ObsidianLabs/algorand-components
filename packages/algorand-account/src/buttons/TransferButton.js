...
export default class TransferButton extends PureComponent {
  ...
  refresh = async () => {
    ...
    if (!value || !algorandNode.algoSdk.isValidAddress(value)) {
      return
    }

    let account
    try {
      account = await algorandNode.algoSdk.accountFrom(value)
      if (account.assets) {
  ...
  push = async () => {
    ...
    try {
      let result
      if (!assetId) {
        result = await algorandNode.algoSdk.transfer({
          from: this.props.address,
          to: recipient,
          amount: Number(amount)
        }, signatureProvider)
      } else {
        result = await algorandNode.algoSdk.transferAsset({
