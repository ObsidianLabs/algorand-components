...
class AlgorandCompiler {
  constructor () {
    this.channel = new IpcChannel('algorand-pyteal')
    this._terminal = null
    this._button = null
  ...
  get projectRoot () {
    if (!this._terminal) {
      throw new Error('AlgorandCompilerTerminal is not instantiated.')
    }
  ...
  get nodeVersion () {
    if (!this._button) {
      throw new Error('AlgorandCompilerButton is not instantiated.')
    }
...
export default new AlgorandCompiler()