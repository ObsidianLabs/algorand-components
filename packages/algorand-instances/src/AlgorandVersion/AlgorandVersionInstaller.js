...
export default class AlgorandVersionInstaller extends PureComponent {
  constructor (props) {
    super(props)
  ...
  fetchRemoteVersions = async () => {
    ...
    let versions
    try {
      versions = await algorandInstancesChannel.invoke('remoteVersions', 10)
    } catch (e) {
  ...
  renderVersions = () => {
    ...
    if (loading) {
      return (
        <DropdownItem key='icon-loading-algorand-versions'>
  ...
  render () {
    const title = (
      <div key='icon-downloading-algorand'>
        <i className='fas fa-spinner fa-spin mr-2' />Downloading Algorand {this.state.downloadVersion}...
      </div>
    )
    return (
      <React.Fragment>
