...
import algorandCompiler from '@obsidians/algorand-compiler'
import algorandKeypair, { signatureProvider } from '@obsidians/algorand-keypair'
import algorandNode from '@obsidians/algorand-node'

class AlgorandProjectManager {
  constructor () {
    this.project = null
    this.modal = null
    this.button = null
  ...
  async checkSettings () {
    ...
    const settings = await this.project.algorandSettings.readSettings()
    return settings
  }
  ...
  async compile () {
    ...
    try {
      await algorandCompiler.build(settings)
    } catch (e) {
      return false
    }
  ...
  async testTransaction (testFile) {
    ...
    const algoTxn = algorandNode.algoSdk.newTransaction(txn, signatureProvider)
